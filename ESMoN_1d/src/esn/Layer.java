...
{	
	...
	protected Layer(int rows, int cols, int num_node, double [] bounds)
	{
		...
		_nodes         = new Matrix(_rows, 1);
		_nodes_history = null;
		_idx_history   = 0;
		_nodes_init    = new Matrix(_rows, 1);
		_nodes_tmp     = new Matrix(_rows, 1);
		_init_state    = new Matrix(_rows, 1);
	 ...
	 */
	public abstract void setActiveWeights();
	...
	public void setActivation(Activation[] activation)
	{
		...
		//check the input
		num_nodes = _nodes.getRowDimension();
		if(activation.length!=num_nodes)
		{
	...
	public void setActivation(activation_E[] activation)
	{
		...
		for(i=0; i<num_nodes; i++)
		{
			...
			if(activation[i]==activation_E.LOGISTIC)
			{
				System.err.println("Layer.setActivation: setting the logistic function is not implemented");
				System.exit(1);
			}
	...
	public void getNodesHistory(int index, double[] storage)
	{
		...
		{
			storage[i] = _nodes_history[index].get(0, i);
		}
	}
	...
	public interval_C[][] getMinMaxState()
	{
		...
		min_max_state = new interval_C[_rows][];
		for(i=0; i<_rows; i++)
		{

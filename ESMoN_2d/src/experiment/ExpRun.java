...
public class ExpRun extends Thread
{
	...
	String[] _file_ffann_wout0_load;//array of paths, including the file names, to FF-ANN modules for output 0
	String[] _file_ffann_wout1_load;//array of paths, including the file names, to FF-ANN modules for output 1
	...
	private void initRunAfterStart(int num_out, int num_sub, int num_intervals)
	{
		...
		{
			_stat_seq[i] = new stat_seq_C(0);
			_stat_seq[i].comp_ident_ok_incl_error       = 0;
			_stat_seq[i].comp_ident_ok = 0;
			_stat_seq[i].mse   = Double.NaN;
			_stat_seq[i].nrmse = Double.NaN;
			_stat_seq[i].rmse  = Double.NaN;
			_stat_seq[i].sel   = Integer.MAX_VALUE;
		}
	...
	private boolean switchRun(mESN esn)
	{
		...
		for(idx_interval=0; idx_interval<_seq_active.getSize(); idx_interval++)
		{
			...
			{
				case 'w':
					markAllModulesActive(max_idx_module);
				...
				break;
				case 'c':
					//indicators of active modules can be reset as a result of configuration
					markAllModulesActive(max_idx_module);
			...
			_exp_output.setRunPerform(stat_run_E.SR_MSE,   idx_interval, _idx_run, _stat_seq[idx_interval].mse  );
			_exp_output.setRunPerform(stat_run_E.SR_NRMSE, idx_interval, _idx_run, _stat_seq[idx_interval].nrmse);
			_exp_output.setRunPerform(stat_run_E.SR_RMSE,  idx_interval, _idx_run, _stat_seq[idx_interval].rmse );
			_exp_output.setRunPerform(stat_run_E.SR_SEL,   idx_interval, _idx_run, _stat_seq[idx_interval].sel  );
			_exp_output.setRunPerform(stat_run_E.SR_LEL,   idx_interval, _idx_run, _stat_seq[idx_interval].lel  );
		}//for "idx_interval"
		return train_ok;
	}
			                        ...
			                        MathDistortion ofb_distort, stat_seq_C stat_seq_store)
	{
		...
		double[]   output_bias;//constant bias at each dimension of the training sequence
		...
		if(is_config==true)//configuration run
		{
			...
			switch(config_method)
			{
				...
				case CM_DiffEvolution:
					...
					//get the indicators which modules were deactivated under synchronization
					esn.GetModuleActiveIndicator(_active_modules);
			    break;
				default:
		...
		else//washout run
		{
			...
			//it is enough to subtract of the largest sequence because only it is used in the "esn.runTeacher" below
			//Later, the output bias is added again in the following function "esn.runTeacher()" before saving.
			output_bias = seq[max_idx_seq].getBias();
			seq[max_idx_seq].subtractBias(output_bias);
	...
	private boolean doTrainRun(mESN esn, int max_idx_seq, int idx_first, int idx_last, stat_seq_C stat_seq_store)
	{
		...
		{
			seq[i]        = _exp_seq.getSeq(i, idx_first,   idx_last);
			//subtraction of output bias is done in the called routines "run with teacher" and "free run" 
			
			seq_init[i]   = _exp_seq.getSeq(i, idx_first-1, idx_first-1);
			//subtraction of output bias is done here because it is not used in the called routines "run with teacher"
			//and "free run" below
			output_bias = seq_init[i].getBias();
			seq_init[i].subtractBias(output_bias);
		}
		...
		for(i=0; i<=max_idx_seq && train_ok==true; i++)
		{
			...
			else
			{
				...
				//restore a bias in the sequence after it has been used
				output_bias = seq_init[i].getBias();
				seq_init[i].addBias(output_bias);
			}
			
			//train the weights of the current sub-reservoir on its sequence
			output_bias = seq[i].getBias();
			seq[i].subtractBias(output_bias);
			esn_sub.training(seq[i]);//subtract known output bias from the training sequence
			train_ok &= esn_sub.getOutputLayer().isTrained();
			
			//store training sequence parameters in the module
			esn_sub.setSeqParam(seq[i].getSeqParametersAsStr());
			
			//store the output bias in the module
			esn_sub.setOutputBias(output_bias);
	 ...
	 */
	private void markAllModulesActive(int max_idx_active)
	{
		int i;
		...
		do{//main loop of the thread
			...
			if(_do_run)
			{
				do{
					...
					esn = new mESN(_file_esn_load, _file_ffann_wout0_load, _file_ffann_wout1_load,
							       _exp_param, _exp_output, _exp_output.getCommonPathRun(_idx_exp, _idx_run), _seed_curr, _seed_load);
	...
	/**
	 * The function uses a provided directory path and provided file names to set paths to files with FF-ANN modules
	 * to be loaded for the specified output.
	 * The function reset a path if there is no request to load the corresponding module.
	 * 
	 * @param path_dir: path to a directory with ESN modules to be loaded
	 * @param ffann_files: array of file names with FF-ANN modules to be loaded
	 * @param load_module: array of requests to load the modules
	 * @param idx_output: index of the ESN output for which the FF-ANN modules must be
	 */
	public void setFileFfannLoad(String path_dir, String[] ffann_files, boolean[] load_module, int idx_output)
	{
		int i;
		int num_files;//number of ESN files to be loaded
		String[] file_ffann_load;//identifier of the array with FF-ANN for the specified ESN output
		
		num_files = ffann_files.length;
		
		//assign a proper storage of the FF-ANN file names
		switch(idx_output)
		{
			case 0:
				_file_ffann_wout0_load = new String[num_files];
				file_ffann_load = _file_ffann_wout0_load;
				break;
			case 1:
				_file_ffann_wout1_load = new String[num_files];
				file_ffann_load = _file_ffann_wout1_load;
				break;
			default:
				file_ffann_load = null;
				System.err.println("ExpRun.setFileFfannLoad: FF-ANN are loaded only for 2 outputs");
				System.exit(1);
				break;
		}
		...
		for(i=0; i<num_files; i++)
		{
			if(load_module[i]==true)
			{
				file_ffann_load[i] = path_dir + File.separator + ffann_files[i];
			}
			else
			{
				file_ffann_load[i] = path_dir + File.separator + "*";
			}
		}
	}

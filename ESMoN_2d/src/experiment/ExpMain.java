...
public class ExpMain
{
	...
	private void doExperiment()
	{
		...
		String[] ffann_files_wout0;//array of paths, including filenames, to FFANN files for output weights for output 0
		String[] ffann_files_wout1;//array of paths, including filenames, to FFANN files for output weights for output 1
		...
		for(i = 0; i < num_vect; i++)
		{
			...
			ffann_files_wout0 = ((vector_C)_exp_param.getParamVal(exp_param_E.EP_IO_LOAD_FFANN_FILE_OUT_0, req_val_E.RV_CUR)).getArrayStr();
			ffann_files_wout1 = ((vector_C)_exp_param.getParamVal(exp_param_E.EP_IO_LOAD_FFANN_FILE_OUT_1, req_val_E.RV_CUR)).getArrayStr();
			...
			while(all_runs_done==false)
			{
				if(j<num_runs)
				{
					...
					if(exp_run!=null)
					{
						...
						exp_run.setFileFfannLoad(_exp_param.getPathEsn(), ffann_files_wout0, load_esn, 0);
						exp_run.setFileFfannLoad(_exp_param.getPathEsn(), ffann_files_wout1, load_esn, 1);
			...
			if(idx_interv_cfg!=-1)
			{
				...
				tmp_double = StatCommon.computeMean(_exp_output.getRunPerform(stat_run_E.SR_SEL, idx_interv_cfg));
				_exp_output.setStatExp(stat_exp_E.SE_SEL_AVG_CFG,       i, tmp_double);
				...
				idx_stat = _math_stat.findMin(_exp_output.getRunPerform(stat_run_E.SR_MSE, idx_interv_cfg));
				_exp_output.setStatExp(stat_exp_E.SE_MIN_MSE_CFG,     i, idx_stat.val);
				_exp_output.setStatExp(stat_exp_E.SE_IDX_MIN_MSE_CFG, i, idx_stat.idx);
				idx_stat = _math_stat.findMax(_exp_output.getRunPerform(stat_run_E.SR_SEL, idx_interv_cfg));
				_exp_output.setStatExp(stat_exp_E.SE_MAX_SEL_CFG,     i, idx_stat.val);
				_exp_output.setStatExp(stat_exp_E.SE_IDX_MAX_SEL_CFG, i, idx_stat.idx);
			...
			else
			{
				...
				do_save[stat_exp_E.SE_SEL_AVG_CFG.ordinal()]        = false;
				...
				do_save[stat_exp_E.SE_MIN_MSE_CFG.ordinal()]        = false;
				do_save[stat_exp_E.SE_IDX_MIN_MSE_CFG.ordinal()]    = false;
				do_save[stat_exp_E.SE_MAX_SEL_CFG.ordinal()]        = false;
				do_save[stat_exp_E.SE_IDX_MAX_SEL_CFG.ordinal()]    = false;

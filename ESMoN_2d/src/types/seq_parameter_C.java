...
{
	private Vector<String> _oscillator_param_as_str;//array of strings containing oscillator parameters
	...
	private class oscillator_param_C
	{
		...
		/**
		 * The function returns a value of a specified parameter.
		 * The parameter is specified by its name.
		 * The function returns NaN if the parameter name is not found among sequence parameters.
		 * The function terminates the program if a parameter value is not a floating point value or multiple parameter
		 * values have been found.
		 * 
		 * @param param_name: specified parameter name
		 * @return: parameter value
		 */
		public double getParameterAsDouble(String param_name)
		{
			int i;
			String tmp_str;
			double param_val;
			
			param_val = Double.NaN;
			for(i=0; i<_parameter_set.size(); i++)
			{
				tmp_str = _parameter_set.get(i)._name;
				if(tmp_str.contentEquals(param_name)==true)
				{
					//check whether a parameter has already been extracted
					if(Double.isNaN(param_val)==true)
					{
						try{
							param_val = new Double(_parameter_set.get(i)._value);
						}
						catch (NumberFormatException nfe)
						{
							System.err.println("getParameterAsDouble: parameter value is not numeric.");
							System.exit(1);
						};
					}
					else
					{
						System.err.println("getParameterAsDouble: multiple parameter values found");
						System.exit(1);
					}
				}
			}
	...
	public seq_parameter_C()
	{
		...
		_oscillator_param_as_str = new Vector<String>(0, 1);
	}
	...
	public seq_parameter_C(Vector<String> param_str)
	{
		...
		for(i=0; i<param_str.size(); i++)
		{
			...
			_oscillator_param_as_str.add(param_str.get(i));
		}
	}
	...
	public void addOscillatorParam(String param_str)
	{
		...
		_oscillator_param.add(param);
		_oscillator_param_as_str.add(param_str);
	}
	...
	public void addOscillatorParam(seq_parameter_C seq_param)
	{
		...
		for(i=0; i<seq_param._oscillator_param.size(); i++)
		{
			...
			param_as_str = seq_param._oscillator_param_as_str.get(i);
			_oscillator_param_as_str.add(param_as_str);
		}
	}
	 ...
	 * dynamics in the target time-series.
	 *  
	 * @return: dynamics' parameters as an array of strings
	 */
	public Vector<String> getSeqParametersAsStr()
	{
		return _oscillator_param_as_str;
	}
	
	/**
	 * The function indicates a number of oscillators in the sequence.
	 * 
	 * @return: number of oscillators
	 */
	public int getNumOscilators()
	{
		return _oscillator_param.size();
	}
	
	/**
	 * The function returns a constant bias for each dimension of the specified oscillator in the sequence.
	 * The bias values are stored in the provided storage.
	 * 
	 * @param idx_osci: index of specified oscillator
	 * @return array of output bias
	 */
	public void getSeqOutputBiasAsDouble(int idx_osci, double[] bias)
	{
		int i;
		String param_name;
		
		//bias name does not have an index if there is a single output dimension
		param_name = "m";
		if(bias.length==1)
		{
			bias[0] = _oscillator_param.get(idx_osci).getParameterAsDouble(param_name);
		}
		else
		{
			for(i=0; i<bias.length; i++)
			{
				//bias parameter name for the current component is "m" + "i"
				//"+1" because the parameter index starts with "1"
				bias[i] = _oscillator_param.get(idx_osci).getParameterAsDouble(param_name + (i + 1));
			}
		}
	}
}
